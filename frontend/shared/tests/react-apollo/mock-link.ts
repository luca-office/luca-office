...
import {GraphQLRequest, FetchResult, ApolloLink, Operation, Observable} from "@apollo/client"
...
export class MockLink extends ApolloLink {
  ...
  public request(operation: Operation) {
    ...
    if (!responses || responses.length === 0) {
      throw new Error(
        `No more mocked responses for the query: ${print(operation.query)}, variables: ${JSON.stringify(
          operation.variables
        )}`
      )
    }

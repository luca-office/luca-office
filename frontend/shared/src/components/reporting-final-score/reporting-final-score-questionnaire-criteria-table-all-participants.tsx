...
import {ColumnProps, Heading, Icon, PercentagePaperPrefix, Table, Text, Tooltip} from "../../components"
...
export type SelectedCriteriaIdsByParticipantId = {[id: string]: UUID[]}
export type NoCriterionFulfilledByParticipantId = {[id: string]: boolean}
export type CriterionCountById = {[id: string]: number}

interface Props<T extends BaseCodingCriterion> extends CustomStyle {
  readonly t: LucaTFunction
  readonly criteria: T[]
  readonly selectedCriteriaIds: UUID[]
  readonly selectedCriterionCountById: CriterionCountById
...
export const ReportingCriteriaTableAllParticipants = <T extends BaseCodingCriterion>(props: Props<T>) => {
  const {
    t,
    customStyles,
    criteria,
    selectedCriterionCountById,
    selectedCriteriaIds,
    totalParticipants,
    scoringType,
    customTitleKey,
  ...
  const isNoCriterionFulfilledItemVisible = !isHolisticScoring && noCriterionFulfilledCount > 0
      ...
      return (
        <div css={Flex.row}>
          <PercentagePaperPrefix
            ratingsCount={selectedCriterionCountById[criterion.id] ?? 0}
            ratersCount={totalParticipants}
          />

          <div css={styles.scoreColumnContent}>

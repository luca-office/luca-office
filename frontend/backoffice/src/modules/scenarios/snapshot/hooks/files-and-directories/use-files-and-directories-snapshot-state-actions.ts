...
export const useFilesAndDirectoriesSnapshotStateActions = (): FilesAndDirectoriesStateActions => {
  ...
  const openTextDocumentInternal = (textDocumentId: string, title?: string) =>
    dispatch(openTextDocument(textDocumentId))

  const resetEmailFileDownloadCounter = noop

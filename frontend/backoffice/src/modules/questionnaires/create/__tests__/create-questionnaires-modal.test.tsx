// importing from direct file because of issues of babel loader and spyOn
import {mount, shallow} from "enzyme"
import * as React from "react"
import {create} from "react-test-renderer"
import {Modal, Overlay, TextArea, TextInput} from "shared/components"
import {mockedFormMethods} from "sharedTests/utils/form-methods-mock"
import * as useCreateQuestionnaireModal from "../hooks/use-create-questionnaire-modal"
import {UseCreateQuestionnaireHook} from "../hooks/use-create-questionnaire-modal"
import {CreateQuestionnaireModal} from "../questionnaire-create-modal"

const hookValuesDefault: UseCreateQuestionnaireHook = {
  createQuestionnaire: jest.fn(),
  createQuestionnaireLoading: false,
  formMethods: mockedFormMethods,
  dismissModal: jest.fn()
}

const stateSpy = jest.spyOn(useCreateQuestionnaireModal, "useCreateQuestionnaireModal")

describe("create-questionnaire-modal", () => {
  it("renders correctly", async () => {
    stateSpy.mockReturnValue(hookValuesDefault)
    const component = create(<CreateQuestionnaireModal />)

    const tree = component.toJSON()
    expect(tree).toMatchSnapshot()
  })

  it("has correct structure", async () => {
    stateSpy.mockReturnValue({...hookValuesDefault})

    const component = shallow(<CreateQuestionnaireModal />)
    expect(component.find(Overlay)).toHaveLength(1)
    expect(component.find(Modal)).toHaveLength(1)
    expect(component.find(TextInput)).toHaveLength(1)
    expect(component.find(TextArea)).toHaveLength(1)
  })

  it("triggers dismiss correctly", async () => {
    const dismissModal = jest.fn()
    const createEvent = jest.fn()
    const handleSubmit = jest.fn()
    stateSpy.mockReturnValue({
      ...hookValuesDefault,
      dismissModal,
      createQuestionnaire: createEvent,
      formMethods: {...mockedFormMethods, handleSubmit} as any
    })

    const component = mount(<CreateQuestionnaireModal />)
    const onDismissHandler = component.find(Modal).prop("onDismiss")
    if (onDismissHandler) {
      onDismissHandler()
      expect(dismissModal).toHaveBeenCalledTimes(1)
    }
  })

  it("does not submit with field errors", async () => {
    const handleSubmit = jest.fn()
    const createEvent = jest.fn()
    stateSpy.mockReturnValue({
      ...hookValuesDefault,
      createQuestionnaire: createEvent,
      formMethods: {
        ...mockedFormMethods,
        errors: {title: undefined, description: {type: "error", message: "Error at Field"}},
        handleSubmit
      } as any
    })

    const component = mount(<CreateQuestionnaireModal />)

    component.find(".confirm-button").last().simulate("click")
    expect(handleSubmit).toHaveBeenCalledTimes(1)
    expect(createEvent).toHaveBeenCalledTimes(0)
  })
})
